...
if X_raw.empty:
    raise ValueError("DataFrame признаков пуст! Проверьте, совпадают ли временные диапазоны логов.")
...
print("\nМатрица ошибок в тексте:")
print(f"True Negative (Плавный просмотр предсказан верно): {cm[0][0]}")
print(f"False Positive (Ложная паника — предсказали зависание, но его нет): {cm[0][1]}")
print(f"False Negative (Пропуск зависания — плеер завис, но мы не ждали): {cm[1][0]}")
print(f"True Positive (Зависание предсказано верно): {cm[1][1]}")
